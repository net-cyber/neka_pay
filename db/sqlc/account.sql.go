...
const topUpAccount = `-- name: TopUpAccount :one
UPDATE accounts 
...
func (q *Queries) TopUpAccount(ctx context.Context, arg TopUpAccountParams) (Account, error) {
	row := q.db.QueryRowContext(ctx, topUpAccount, arg.Amount, arg.ID)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
